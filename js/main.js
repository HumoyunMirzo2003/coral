...
let pokBox = document.querySelector('#pokemonbox');
...
pokemons.forEach((el) => {
	...
	let elHour = document.createElement('p');
	let elHeight = document.createElement('p');
	let elWeight = document.createElement('p');
	let elWeak = document.createElement('p');

	pokDiv.classList.add("box");
	pokDiv.classList.add("col-lg-3");
	pokDiv.classList.add("col-md-4");
	pokDiv.classList.add("col-sm-8");
	pokEl.classList.add("pokEl");
	elId.classList.add("id");
  	elWeak.classList.add('weak')

  	elImg.src = el.img;
  	elId.textContent = el.id;
  	elName.textContent = el.name;
  	elHour.textContent = el.spawn_time;
  	elHeight.textContent = `Height: ${el.height}`;
  	elWeight.textContent = `Weight: ${el.weight}`;
  	elWeak.textContent = el.weaknesses;



	pokEl.append(elId, elName, elHour, elHeight, elWeight);
	pokDiv.append(elImg, pokEl, elWeak);

	pokBox.appendChild(pokDiv);
});
...
// 	var filter = search.value.toLocaleLowerCase();
